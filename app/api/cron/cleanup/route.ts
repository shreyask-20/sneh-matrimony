...
export async function GET(request: NextRequest) {
  try {
    ...
    // 2. Finalize account deletions where the 5-day grace period has expired
    const graceExpiry = new Date(Date.now() - 5 * 24 * 60 * 60 * 1000);
    const finalizedDeletions = await prisma.user.updateMany({
      where: {
        deletionRequestedAt: { lte: graceExpiry },
        deletedAt: null,
      },
      data: {
        deletedAt: new Date(),
      ...
      html: `
        <div style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:32px 24px;background:#fff;border-radius:16px;">
          ...
          <table style="width:100%;border-collapse:collapse;">
            ...
            <tr style="background:#fdf2f6;">
              <td style="padding:12px 16px;font-size:14px;color:#475569;border-radius:8px 0 0 8px;">Accounts Finalized (deletion grace expired)</td>
              <td style="padding:12px 16px;font-size:18px;font-weight:700;color:${finalizedDeletions.count > 0 ? "#dc2626" : "#1e0a14"};text-align:right;border-radius:0 8px 8px 0;">${finalizedDeletions.count}</td>
            </tr>
          </table>
    ...
    return NextResponse.json({
      ...
      finalizedDeletions: finalizedDeletions.count,
